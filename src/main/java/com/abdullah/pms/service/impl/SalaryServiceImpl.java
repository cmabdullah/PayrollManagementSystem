...
public class SalaryServiceImpl implements SalaryService {
	...
	public void calculateSalary() {
		...
		List<UserInfo> userEnableInfo = UserInfoService.findAll();
		userEnableInfo.forEach(n -> System.out.println(n.toString()));
		UserInfo userInfo = userEnableInfo.get(3);
		Grade grade = userEnableInfo.get(0).getGrade();

		// calculate bonus
		float totalsalary = grade.getBasic() + grade.getHouseRent() + grade.getLunch() + grade.getMedicalAllowence()
				+ grade.getStudy() + grade.getTransport();

		float oneDaySalary = totalsalary / 22;
		...
		Optional<List<Attendance>> usersAllAttendanceList = Optional.ofNullable(
				attendanceService.findByUserInfoAndLoginDateBetween(userEnableInfo.get(0), localDateFrom, localDateTo));
		...
		float finalSalary = countMonthlySalary( totalsalary,  oneDaySalary, monthlyWorkingHour  );
		Optional<Loan> loan = Optional.ofNullable(loanService.runningLoanObject(userInfo));
		double installment = 0;//kisti
		if (loan.isPresent()) {
			LoanPaidDetails loanPaidDetails = getLoanPaidDetails(loan.get(), finalSalary);
			loanPaidDetailsService.save(loanPaidDetails);
			installment = loanPaidDetails.getInstallment();
			
		}//loan service end
		
		
		//loan,leave
		Salary salary = Salary.builder().userInfo(userInfo).grade(grade).monthlyWorkingHour(monthlyWorkingHour).totalLeaveDays(numberOfLeaveDays).regular("y").totalSalary(finalSalary).netSalary((float)(finalSalary-installment)).paymentDate(new Date()).build();
	...
	private LoanPaidDetails getLoanPaidDetails(Loan loan, float finalSalary) {
		...
		return  LoanPaidDetails.builder().installmentDate(new Date()).loan(loan)
		.installment((float)installment).paidAmount((float)paidAmount).build();
